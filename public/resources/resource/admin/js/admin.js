...
{
    $('.tagprogress').val(0);
    $('.tag_progress span').html('отчет загруженных файлов');
}


function imageupload() {
    var file = _("archiveimage").files[0];
    var formdata = new FormData();
    formdata.append("archiveimage", file);
    var ajax = new XMLHttpRequest();
    ajax.upload.addEventListener("progress", progressHandlers, false);
    ajax.open("POST", "");
    ajax.send(formdata);
}

function progressHandlers(event) {
    _("uploadstatusimage").innerHTML = "Uploaded " + event.loaded + " bytes of " + event.total;
    var percent = (event.loaded / event.total) * 100;
    _("progressBarimage").value = Math.round(percent);
}